...
{
    return ((int32_t)turnAngle >> 16) * 360 >> 16; // converts 32-bit fixed-point to int degrees
}
...
bool TurnByDegree(int32_t UserDegreeTurn2, int userDirection, bool runOnce)
{
    ...
    {
        case 0: return currentAngle > RightCalibratedTurn; // right turn not complete?
        case 1: return currentAngle < LeftCalibratedTurn;  // left turn not complete?
...
void DriveTurn(int32_t userDegreeTurn3, int userDirection)
{
    ...
    while (t <= secondsToDegree(userDegreeTurn3) && angleBreakpointReached)
    {
        ...
        delay(20);               // control loop interval ~50 Hz
